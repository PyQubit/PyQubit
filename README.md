<div align="center">

<img
  src="https://capsule-render.vercel.app/api?type=waving&color=0:050508,45:0D1117,100:050508&height=220&section=header&text=MmdxDev&fontSize=72&fontColor=FFD700&animation=fadeIn&fontAlignY=35&desc=AI%20Engineer%20%26%20Data%20Scientist&descAlignY=57&descSize=19&descColor=00D2FF"
  width="100%"
  alt="MmdxDev"
 />

<h2>Mohammad Mahdi Omidvar</h2>

<p>
<strong>AI Engineer & Data Scientist</strong>
</p>

<p>
Building intelligent solutions with Python, Machine Learning & Deep Learning.<br/>
Focused on NLP, Computer Vision, Data Science and production-ready AI systems.
</p>

<br/>

<img
  src="https://readme-typing-svg.demolab.com/?font=Fira+Code&weight=600&size=23&duration=3000&pause=1100&color=FFD700&center=true&vCenter=true&width=850&lines=AI+Engineer+%26+Data+Scientist;Building+Intelligent+Solutions+with+Python;Machine+Learning+%26+Deep+Learning;NLP+%26+Computer+Vision;Research+%E2%86%92+Engineering+%E2%86%92+Production"
  alt="Typing Animation"
/>

<br/><br/>

<a href="https://pyqubit.github.io/">
  <img src="https://img.shields.io/badge/Portfolio-050508?style=for-the-badge&logo=googlechrome&logoColor=FFD700" alt="Portfolio"/>
</a>
&nbsp;
<a href="https://www.linkedin.com/in/pyqubit">
  <img src="https://img.shields.io/badge/LinkedIn-050508?style=for-the-badge&logo=linkedin&logoColor=00D2FF" alt="LinkedIn"/>
</a>
&nbsp;
<a href="https://github.com/PyQubit">
  <img src="https://img.shields.io/badge/GitHub-050508?style=for-the-badge&logo=github&logoColor=FFD700" alt="GitHub"/>
</a>
&nbsp;
<a href="https://t.me/PyQubit">
  <img src="https://img.shields.io/badge/Telegram-050508?style=for-the-badge&logo=telegram&logoColor=00D2FF" alt="Telegram"/>
</a>
&nbsp;
<a href="mailto:pyqubit@gmail.com">
  <img src="https://img.shields.io/badge/Email-050508?style=for-the-badge&logo=gmail&logoColor=FFD700" alt="Email"/>
</a>

<br/><br/>

<img
  src="https://komarev.com/ghpvc/?username=PyQubit&label=PROFILE%20VIEWS&color=FFD700&style=for-the-badge"
  alt="Profile Views"
/>

</div>

---

## 🧠 About

I'm an **AI Engineer & Data Scientist** focused on turning ideas, algorithms and data into practical intelligent systems.

My work sits at the intersection of:

`AI` · `Machine Learning` · `Deep Learning` · `NLP` · `Computer Vision` · `Data Science` · `Python`

I enjoy building systems that move beyond experimentation and toward real-world applications.

```text
Idea
  ↓
Research
  ↓
Model Development
  ↓
System Engineering
  ↓
Deployment
  ↓
Real-World Impact
````

---

## ⚡ What I Build

<table>
<tr>

<td width="50%" valign="top">

### 🤖 AI & Machine Learning

Developing intelligent systems using classical Machine Learning and modern Deep Learning techniques.

</td>

<td width="50%" valign="top">

### 🧠 Deep Learning

Designing and training neural network architectures with frameworks such as **PyTorch** and **TensorFlow**.

</td>

</tr>

<tr>

<td width="50%" valign="top">

### 👁️ Computer Vision

Working with image understanding, classification, object detection, segmentation and real-time vision systems.

</td>

<td width="50%" valign="top">

### 🗣️ NLP & Transformers

Building NLP systems with modern Transformer architectures, LLMs and conversational AI pipelines.

</td>

</tr>

<tr>

<td width="50%" valign="top">

### 📊 Data Science

Transforming raw data into useful insights through cleaning, EDA, feature engineering and predictive modeling.

</td>

<td width="50%" valign="top">

### ⚙️ AI Engineering

Integrating AI models into scalable APIs and production-oriented applications using Python backend technologies.

</td>

</tr>
</table>

---

## 🛠️ Tech Stack

<div align="center">

### Languages & Core

<img src="https://skillicons.dev/icons?i=python&theme=dark" height="52" alt="Python"/>

<br/><br/>

### AI / Machine Learning

<img src="https://skillicons.dev/icons?i=pytorch,tensorflow,sklearn,opencv,numpy,pandas&theme=dark" height="52" alt="AI Stack"/>

<br/><br/>

### Backend & Engineering

<img src="https://skillicons.dev/icons?i=fastapi,django,flask,postgres,mysql,docker,git,github,linux&theme=dark" height="52" alt="Backend Stack"/>

</div>

---

## 🏆 Featured Achievement

<div align="center">

### 🥇 INNOVERSE 2025 — Gold Medalist

<strong>AI Section · AI Emotion-to-Art Project</strong>

<br/><br/>

A multimodal AI system designed to understand human emotions from:

`Text` · `Voice` · `Image`

and transform emotional information into generative artwork.

<br/><br/>

<img
src="https://pyqubit.github.io/3.jpg"
width="420"
alt="INNOVERSE 2025 Gold Medal"
/>

<br/><br/>

<strong>Team</strong>

Mohammad Mahdi Omidvar · Mahyar Alizadeh · Arad Chizari · Sogol Tarnabi

<br/><br/>

<a href="https://github.com/PyQubit/Innoverse-2025">
  <img
    src="https://img.shields.io/badge/VIEW%20PROJECT-050508?style=for-the-badge&logo=github&logoColor=FFD700"
    alt="View Project"
  />
</a>

</div>

---

## 🚀 Flagship Project

### Innoverse-2025 — AI Emotion-to-Art

A multimodal AI pipeline combining emotion understanding, speech processing and generative AI.

### Tech Stack

`Python` · `Gradio` · `Ollama` · `LLMs` · `Whisper` · `Audio` · `Replicate API`

### Architecture

```text
                 ┌─────────────┐
Text ───────────►│             │
                 │   Emotion   │
Voice ──────────►│ Understanding│───► AI Interpretation
                 │             │
Image ──────────►│             │
                 └─────────────┘
                         │
                         ▼
                Generative Artwork
```

### Key Features

* Multimodal emotion recognition
* Speech-to-text processing
* AI-based emotion interpretation
* Generative artwork synthesis
* Interactive AI interface

<div align="center">

<a href="https://github.com/PyQubit/Innoverse-2025">
  <img
    src="https://img.shields.io/badge/VIEW%20SOURCE%20CODE-050508?style=for-the-badge&logo=github&logoColor=FFD700"
    alt="View Source Code"
  />
</a>

</div>

---

## 📜 Certifications

<div align="center">

<table>
<tr>

<td align="center" width="50%">

<img
src="https://pyqubit.github.io/py.png"
width="360"
alt="Basic Python"
/>

<br/>

<strong>Basic Python</strong>

</td>

<td align="center" width="50%">

<img
src="https://pyqubit.github.io/pyad.png"
width="360"
alt="Advanced Python"
/>

<br/>

<strong>Advanced Python</strong>

</td>

</tr>

<tr>

<td align="center" width="50%">

<img
src="https://pyqubit.github.io/al.png"
width="360"
alt="AI Algorithms"
/>

<br/>

<strong>AI Algorithms</strong>

</td>

<td align="center" width="50%">

<img
src="https://pyqubit.github.io/dl.png"
width="360"
alt="Deep Learning"
/>

<br/>

<strong>Deep Learning</strong>

</td>

</tr>

<tr>

<td align="center" width="50%">

<img
src="https://pyqubit.github.io/ds.png"
width="360"
alt="Data Science"
/>

<br/>

<strong>Data Science</strong>

</td>

<td align="center" width="50%">

<img
src="https://pyqubit.github.io/api.png"
width="360"
alt="Advanced API Programming"
/>

<br/>

<strong>Advanced API Programming</strong>

</td>

</tr>
</table>

</div>

---

## 📈 GitHub Analytics

<div align="center">

<img
src="https://github-readme-stats.vercel.app/api?username=PyQubit&show_icons=true&hide_border=true&theme=radical&bg_color=050508&title_color=FFD700&icon_color=00D2FF&text_color=F0F0F0&ring_color=FFD700"
height="180"
alt="GitHub Stats"
/>

<img
src="https://github-readme-streak-stats.herokuapp.com/?user=PyQubit&theme=dark&hide_border=true&background=050508&ring=FFD700&fire=FFD700&currStreakLabel=00D2FF&sideLabels=F0F0F0&dates=A0A0B0"
height="180"
alt="GitHub Streak"
/>

<br/><br/>

<img
src="https://github-readme-stats.vercel.app/api/top-langs/?username=PyQubit&layout=compact&hide_border=true&theme=radical&bg_color=050508&title_color=FFD700&text_color=F0F0F0"
height="180"
alt="Top Languages"
/>

</div>

---

## 🐍 Contribution Activity

<div align="center">

<img
src="https://raw.githubusercontent.com/PyQubit/PyQubit/output/github-contribution-grid-snake-dark.svg"
alt="Contribution Snake"
/>

</div>

---

## 🔭 Currently Exploring

<div align="center">

`LLMs`
·
`Multimodal AI`
·
`AI Agents`
·
`Computer Vision`
·
`Intelligent Automation`
·
`Production AI`

<br/><br/>

```text
Research
   ↓
Model Development
   ↓
System Engineering
   ↓
API / Application
   ↓
Production AI
```

<br/>

<strong>Build. Experiment. Engineer. Ship.</strong>

</div>

---

## 🌐 Connect

<div align="center">

### Let's build something intelligent.

<br/>

<a href="https://pyqubit.github.io/">
  <img src="https://img.shields.io/badge/Portfolio-050508?style=for-the-badge&logo=googlechrome&logoColor=FFD700" alt="Portfolio"/>
</a>

<a href="https://www.linkedin.com/in/pyqubit">
  <img src="https://img.shields.io/badge/LinkedIn-050508?style=for-the-badge&logo=linkedin&logoColor=00D2FF" alt="LinkedIn"/>
</a>

<a href="https://github.com/PyQubit">
  <img src="https://img.shields.io/badge/GitHub-050508?style=for-the-badge&logo=github&logoColor=FFD700" alt="GitHub"/>
</a>

<a href="https://t.me/PyQubit">
  <img src="https://img.shields.io/badge/Telegram-050508?style=for-the-badge&logo=telegram&logoColor=00D2FF" alt="Telegram"/>
</a>

<a href="mailto:pyqubit@gmail.com">
  <img src="https://img.shields.io/badge/Email-050508?style=for-the-badge&logo=gmail&logoColor=FFD700" alt="Email"/>
</a>

<br/><br/>

<strong>Mohammad Mahdi Omidvar · MmdxDev</strong>

<br/>

<em>AI Engineer & Data Scientist · INNOVERSE 2025 Gold Medalist</em>

<br/><br/>

<img
src="https://capsule-render.vercel.app/api?type=waving&color=0:050508,50:0D1117,100:050508&height=120&section=footer&text=Thank%20you%20for%20visiting&fontSize=16&fontColor=A0A0B0&animation=fadeIn"
width="100%"
alt="Footer"
/>

</div>
